...
export const Studentdash = ({ routes }) => {
    ...
    <>
      ...
      <nav className="fixed top-0 z-50 w-full bg-black/95 backdrop-blur-md flex justify-between items-center px-4 py-3 shadow-lg border-b border-purple-800">
        <div className="flex items-center">
          <button className="sm:hidden mr-3 text-purple-400 hover:text-purple-600" onClick={toggleSidebar}>
          ...
          <ul className="space-y-2 font-medium">
            {routes.map((item) => (
              <li key={item.path}>
                <Link
                  to={item.path}
                  className={`flex items-center p-2 text-purple-400 rounded-lg hover:text-pink-400 hover:scale-105 transition-transform duration-200 ${
                    location.pathname.includes(item.path) ? "bg-purple-900/40 shadow-neon" : ""
                  }`}
      ...
      <div className="p-4 sm:ml-64 mt-16 min-h-screen bg-black text-white">
        <div className="p-6 border-2 border-purple-800 border-dashed rounded-lg min-h-[80vh] shadow-neon-glow">
          <Outlet />
        </div>
      </div>
    </>
